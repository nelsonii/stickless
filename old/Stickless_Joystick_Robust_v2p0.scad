...
module contact() {
  ...
  tube(h=contactHeightZ, id=wall-8, wall=8);

  //color("steelblue")  cuboid([10,10,contactHeightZ], fillet=1, p1=[-shift-5, -5, 0]); //-X
  //color("steelblue")  cuboid([10,10,contactHeightZ], fillet=1, p1=[+shift-5, -5, 0]); //+X
  //color("steelblue")  cuboid([10,10,contactHeightZ], fillet=1, p1=[-5, -shift-5, 0]); //-Y
  //color("steelblue")  cuboid([10,10,contactHeightZ], fillet=1, p1=[-5, +shift-5, 0]); //+Y
...
module contactswitch(){
  ...
  translate([-23, -switchWidthX/2, wallThickness]) cube([switchDepthY, switchWidthX, switchHeightZ+2]); //-X
  translate([+12, -switchWidthX/2, wallThickness]) cube([switchDepthY, switchWidthX, switchHeightZ+2]); //+X
  translate([-(switchWidthX/2), +12, wallThickness]) cube([switchWidthX, switchDepthY, switchHeightZ+2]); //+Y
  translate([-(switchWidthX/2), -23, wallThickness]) cube([switchWidthX, switchDepthY, switchHeightZ+2]); //+Y
