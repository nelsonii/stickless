...
module contactswitch(){
  ...
  color("orange") translate([-30.5, -switchWidthX/2, wallThickness]) cube([switchDepthY, switchWidthX, switchHeightZ+2]); //-X
  color("orange") translate([+12, -switchWidthX/2, wallThickness]) cube([switchDepthY, switchWidthX, switchHeightZ+2]); //+X
  color("orange") translate([-(switchWidthX/2), +12, wallThickness]) cube([switchWidthX, switchDepthY, switchHeightZ+2]); //+Y
  color("orange") translate([-(switchWidthX/2), -30.5, wallThickness]) cube([switchWidthX, switchDepthY, switchHeightZ+2]); //+Y
...
module contactswitchslider() {
  ...
  contactHeightZ = caseHeightZ-caseLidHeight-overlap;
  color("lime") tube(h=contactHeightZ, od=fingerSize+(fingerSize*0.20), wall=3);
